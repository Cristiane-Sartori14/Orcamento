...
function gerarPDF() {
  ...
  const opt = {
    margin: [0.8, 0.8, 0.8, 0.8], // margem padrão em cm (superior, dir, inf, esq)
    filename: `Orcamento_${new Date().getFullYear()}_${Date.now()}.pdf`,
    image: { type: "jpeg", quality: 0.98 },
    html2canvas: {
      scale: 3,           // melhor qualidade (ajuste se ficar pesado)
...
function visualizarOrcamentoEmNovaPagina() {
  ...
  // Substitui os inputs por texto simples
  ...
  // Escreve o HTML completo na nova aba
  novaJanela.document.write(`
    <!DOCTYPE html>
    <html lang="pt-BR">
    <head>
      <meta charset="UTF-8" />
      <title>Visualização do Orçamento</title>
      <link rel="stylesheet" href="Styles.css">
      <style>
        body { padding: 20px; font-family: Segoe UI, sans-serif; background: #f9f9f9; }
        #orcamentoArea { box-shadow: 0 0 5px rgba(0,0,0,0.1); }
      </style>
    </head>
    <body>
      ${clone.outerHTML}
