...
linha.querySelector(".valor").addEventListener("blur", () => {
  const raw = linha.querySelector(".valor").value.replace(',', '.');
  const val = parseFloat(raw);
  if (!isNaN(val)) {
    linha.querySelector(".valor").value = val.toFixed(2).replace('.', ',');
  } else {
    linha.querySelector(".valor").value = '0,00';
  }
});
